...
- A minimal local helper-command-contract smoke check is available at `Tests/SecurityHarness/helper_command_contract_smoke.swift` and should be run with `swiftc ClashX/Models/HelperCommandContract.swift ClashX/General/Utils/HelperCommandRegistry.swift Tests/SecurityHarness/helper_command_contract_smoke.swift -o /tmp/helper-command-contract-smoke && /tmp/helper-command-contract-smoke` when changing the typed helper command inventory, reserved TUN command naming, forbidden command categories, or contract serialization.
- A minimal local TUN lifecycle diagnostics smoke check is available at `Tests/SecurityHarness/tun_lifecycle_diagnostics_smoke.swift` and should be run with `swiftc ClashX/General/Utils/ConfigValidationIssue.swift ClashX/General/Utils/TunConfigValidator.swift ClashX/General/Utils/DNSConfigValidator.swift ClashX/Models/HelperStatus.swift ClashX/Models/HelperCommandContract.swift ClashX/General/Utils/HelperCommandRegistry.swift ClashX/Models/TunLifecycleDiagnostics.swift ClashX/General/Managers/TunPreflightPlanner.swift Tests/SecurityHarness/tun_lifecycle_diagnostics_smoke.swift -o /tmp/tun-lifecycle-diagnostics-smoke && /tmp/tun-lifecycle-diagnostics-smoke` when changing preflight blockers, verification-scope wording, or the typed TUN lifecycle diagnostics boundary.
- A minimal local TUN runtime diagnostics smoke check is available at `Tests/SecurityHarness/tun_runtime_diagnostics_smoke.swift` and should be run with `swiftc ClashX/General/Utils/ConfigValidationIssue.swift ClashX/General/Utils/TunConfigValidator.swift ClashX/General/Utils/DNSConfigValidator.swift ClashX/Models/HelperStatus.swift ClashX/Models/HelperCommandContract.swift ClashX/General/Utils/HelperCommandRegistry.swift ClashX/Models/TunLifecycleDiagnostics.swift ClashX/General/Utils/TunRuntimeInterfaceProbe.swift ClashX/General/Managers/TunPreflightPlanner.swift Tests/SecurityHarness/tun_runtime_diagnostics_smoke.swift -o /tmp/tun-runtime-diagnostics-smoke && /tmp/tun-runtime-diagnostics-smoke` when changing read-only interface evidence, tun-like interface classification, or runtime-verification wording.
...
- PR CI runs `security_harness.swift`, `controller_endpoint_builder_smoke.swift`, `capability_cache_identity_smoke.swift`, `config_validator_smoke.swift`, `diagnostics_log_reader_smoke.swift`, `diagnostics_artifact_formatter_smoke.swift`, `profile_artifact_metadata_smoke.swift`, `smartx_managed_override_smoke.swift`, `effective_config_generator_smoke.swift`, `helper_status_smoke.swift`, `helper_command_contract_smoke.swift`, `tun_lifecycle_diagnostics_smoke.swift`, `tun_runtime_diagnostics_smoke.swift`, `redactor_smoke.swift`, and `remote_config_decode_smoke.swift`.
...
- `helper_command_contract_smoke.swift` compiles the production `HelperCommandContract.swift` plus `HelperCommandRegistry.swift` files directly and only checks the typed command inventory, reserved TUN contract boundary, forbidden command notes, and Codable behavior. It is contract-boundary coverage, not a live helper XPC or TUN execution test.
...
- `helper_command_contract_smoke.swift` compiles `HelperCommandContract.swift` plus `HelperCommandRegistry.swift` directly to cover implemented system-proxy commands, reserved TUN commands, forbidden command categories, and Codable round-trip behavior for command descriptors and results.
- `tun_lifecycle_diagnostics_smoke.swift` compiles `TunLifecycleDiagnostics.swift` plus `TunPreflightPlanner.swift` with lightweight stubs to cover embedded-core blocking, controller-not-running blocking, missing tun-section blocking, reserved helper TUN state, controller-config-only verification scope, and report serialization.
- `tun_runtime_diagnostics_smoke.swift` compiles `TunLifecycleDiagnostics.swift`, `TunRuntimeInterfaceProbe.swift`, and `TunPreflightPlanner.swift` with lightweight stubs to cover read-only tun-like interface evidence, conservative runtime-verification messaging, and explicit route/DNS/packet-flow not-implemented levels.
