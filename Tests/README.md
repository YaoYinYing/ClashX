...
- `bash scripts/codex-test-focused.sh` now pre-runs `effective_config_generator_smoke.swift` before the Xcode test invocation so the unsupported/provenance boundary stays covered even when workspace readiness blocks Xcode tests.
