...
The branch now also has a typed TUN lifecycle diagnostics boundary in `TunLifecycleDiagnostics.swift` plus `TunPreflightPlanner.swift`, a read-only runtime interface probe in `TunRuntimeInterfaceProbe.swift`, a read-only route-evidence probe in `TunRuntimeRouteProbe.swift`, and a read-only DNS runtime probe in `TunRuntimeDNSProbe.swift`. That boundary now distinguishes passive snapshot diagnostics, guarded enable requests, and guarded disable requests. Blocking TUN/DNS validation applies only to enable requests, while disable requests keep the same validation findings as warnings unless the controller patch itself is unavailable. Passive snapshots remain explicitly passive and do not present cached/current `tun.enable` as post-toggle verification. Route evidence now reports observed IPv4/IPv6 primary route interfaces instead of calling a sorted first interface the default route. Route evidence is still not packet-flow proof, DNS runtime evidence is still not DNS hijack proof, and packet-flow verification remains unimplemented.
