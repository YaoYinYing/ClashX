...
- `ControllerEndpointBuilder` is implemented, stopped-core results are cleaned up, Smart endpoint results are unified.
- 6 thin wrapper methods deleted from `ApiRequest` (→ ConnectionAPI, SmartAPI). Remaining wrappers have active callers.
- Stream lifecycle (traffic/log WebSocket) still remains in `ApiRequest` while retry ownership is shared with WebSocket delegate state.
...
- the typed helper command contract model and registry now make future helper-backed TUN planning more explicit, but every TUN helper command remains reserved and non-executable in the current branch
...
- the new typed TUN lifecycle diagnostics boundary now distinguishes passive snapshots, enable requests, and disable requests; blocking TUN/DNS validation currently applies only to enable requests, passive snapshots stay explicitly non-verifying, and the read-only interface, route, and DNS runtime evidence still must not be described as packet-flow verification or DNS hijack verification
